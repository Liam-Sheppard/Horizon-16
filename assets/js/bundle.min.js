...
this.workContent=$(workContentContainer).find('.work-content');this.carousel=workContentContainer.find('.work-carousel').slick({infinite:false,autoplay:false,arrows:false,swipeToSlide:true,dots:true,nextArrow:'<span class="slick-next">Next</span>',prevArrow:'<span class="slick-prev">Prev</span>',});this.carousel.find('.work-carousel-item').click(function(e){_this.carousel.slick('slickGoTo',$(this).index());});$(workContentContainer).find('.mt-close-work').click(function(e){e.preventDefault();window.history.back();});this.workContent.addClass('loaded');}
...
if($('.single-work').length){initialiseWorkPage();}});})(jQuery);$(document).ready(function(){$('body').addClass('loaded');var navInTransition=false;function openMobileMenu(){if(!navInTransition){navInTransition=!navInTransition;$('#primary-navigation-menu').show();setTimeout(function(){$('body').addClass('show-mobile-menu');navInTransition=!navInTransition;},50);}}
...
var $scrollBasedAnimates=$('.panel-bridge, .horizon-rule, h2, .next-graduate, .animate-when-seen');watchSeen($scrollBasedAnimates);$(window).scroll(function(){watchSeen($scrollBasedAnimates);});});