...
Egg.prototype.__toCharCodes=function(keys){var special={"up":38,"down":40,"left":37,"right":39,"enter":13,"space":32,"ctrl":17,"alt":18,"tab":9,"esc":27},specialKeys=Object.keys(special);if(typeof keys==='string'){keys=keys.split(',').map(function(key){return key.trim();});}
...
return d.overflowed[0]||d.overflowed[1];}});});}))}));(function($){$(document).ready(function(){var egg=new Egg();var thugLife=new Audio(siteData.themeUri+'/assets/sounds/eggs/thug-life.mp3');egg.addCode("v,o,v,o",function(){$('body').prepend('<div class="ryan"></div>');thugLife.play();setTimeout(function(){$('.ryan').remove();thugLife.pause();thugLife.currentTime=0;},10000);}).addHook(function(){}).listen();console.log('Easter Egg Code: v o v o');var nyanTrack=new Audio(siteData.themeUri+'/assets/sounds/eggs/nyan.mp3');egg.addCode("m,e,o,w",function(){$('body').prepend('<div class="holly"></div>');nyanTrack.play();setTimeout(function(){$('.holly').fadeOut();},5500);setTimeout(function(){$('.holly').remove();nyanTrack.pause();nyanTrack.currentTime=0;},7500);}).addHook(function(){}).listen();console.log('Easter Egg Code: m e o w');var notLikeThis=new Audio(siteData.themeUri+'/assets/sounds/eggs/not-like-this.mp3');var alreadyRip=false;egg.addCode("r,i,p",function(){if(!alreadyRip){alreadyRip=true;notLikeThis.play();setTimeout(function(){$('body').css('font-family','"Comic Sans MS", cursive, sans-serif');},6000);}}).addHook(function(){}).listen();console.log('Easter Egg Code: r i p');egg.addCode("h,e,l,l",function(){$('body').addClass('break-everything');$('body').append('<div class="internetexplorer"></div>');}).addHook(function(){}).listen();console.log('Easter Egg Code: h e l l');egg.addCode("n,o,d",function(){var heroVideo=$('#heroVideo');if(heroVideo.length){heroVideo.find('source[type="video/webm"]').attr('src',siteData.themeUri+'/assets/video/nod.webm');heroVideo.find('source[type="video/mp4"]').attr('src',siteData.themeUri+'/assets/video/nod.mp4');heroVideo.load();}}).addHook(function(){}).listen();console.log('Easter Egg Code: n o d');var rolling=new Audio(siteData.themeUri+'/assets/sounds/eggs/rolling.mp3');egg.addCode("r,o,l,l",function(){$('body').prepend('<div class="datboi"></div>');rolling.play();setTimeout(function(){$('.datboi').remove();rolling.pause();rolling.currentTime=0;},14000);}).addHook(function(){}).listen();console.log('Easter Egg Code: r o l l');egg.addCode("t,h,a,n,k,s",function(){if($('body').find('.balloon-egg').length==0){$('body').prepend(`
            ...
            </div>`);$('.balloon-egg').click(function(e){$(this).remove();})}}).addHook(function(){}).listen();});})(jQuery);(function($){$(document).ready(function(){var strips=$(".strips-container");function initialiseStripsScroll(){strips.mCustomScrollbar({axis:'x',scrollInertia:80,advanced:{updateOnContentResize:true,}});}
function updateStrips(){var vw=$(window).innerWidth();if(vw<700){if(strips.hasClass('mCustomScrollbar')){strips.mCustomScrollbar('destroy');}}else{if(!strips.hasClass('mCustomScrollbar')){initialiseStripsScroll();}
setTimeout(function(){if(strips.hasClass('mCustomScrollbar')){strips.mCustomScrollbar('scrollTo',0);strips.mCustomScrollbar('update');}},100);}}
updateStrips();$(window).on('resize',function(){vw=$(window).innerWidth();updateStrips();});function GraduatesFilter(el){var _this=$(this);_this.mobileFilterButton=$(el).find('.toggle-graduate-filters.button');_this.gradFilters=$(el).find('.graduate-filters');_this.gradFiltersWrapper=$(el).find('.graduate-filters-wrapper');_this.filterButtons=$(el).find('.menu-item');_this.graduates=$('.single-grad');_this.showGraduatesOfClass=function(filterClass){if(filterClass=='all'){_this.graduates.addClass('active')
return _this;}
_this.graduates.removeClass('active');_this.graduates.addClass('hide');setTimeout(function(){_this.graduates.filter('.'+filterClass).each(function(){$(this).addClass('active');$(this).prependTo($(this).parents('.strip'));});},400);setTimeout(function(){_this.graduates.removeClass('hide');},600);return _this;}
_this.setActiveFilter=function(el){_this.filterButtons.filter('.active').removeClass('active');$(el).addClass('active');return _this;}
_this.openMobileFilters=function(){_this.gradFiltersWrapper.show();$('body').addClass('hide-header').addClass('hide-overflow');setTimeout(function(){_this.gradFiltersWrapper.addClass('visible');},50);}
_this.closeMobileFilters=function(){$('body').removeClass('hide-header').removeClass('hide-overflow');_this.gradFiltersWrapper.removeClass('visible');setTimeout(function(){_this.gradFiltersWrapper.hide();},400);}
_this.mobileFilterButton.click(function(e){if(!_this.gradFiltersWrapper.hasClass('visible')){_this.openMobileFilters();}else{_this.closeMobileFilters();}});_this.filterButtons.click(function(e){if(!$(this).hasClass('active')){_this.setActiveFilter(this).showGraduatesOfClass($(this).data('filter-class'));_this.mobileFilterButton.html('Filter: '+$(this).text());if(strips.hasClass('mCustomScrollbar')){strips.mCustomScrollbar('scrollTo',0);}}
if($(this).parents('.graduate-filters-wrapper').hasClass('visible')){_this.closeMobileFilters();}});_this.gradFiltersWrapper.click(function(e){if($(e.target).hasClass('visible')){_this.closeMobileFilters();}});}
var gradFilter=new GraduatesFilter('.graduate-filters-container');});})(jQuery);(function($){$(document).ready(function(){function initialiseWorkPage(workContentContainer){var _this=this;if(!workContentContainer){var workContentContainer=$('body');}
this.workContent=$(workContentContainer).find('.work-content');this.carousel=workContentContainer.find('.work-carousel').slick({infinite:false,autoplay:false,arrows:false,dots:true});this.carousel.find('.work-carousel-item').click(function(e){_this.carousel.slick('slickGoTo',$(this).index());});$(workContentContainer).find('.close-works').click(function(e){e.preventDefault();graduateProfile.closeWorks();});this.workContent.addClass('loaded');}
function graduateProfile(profileIdentifier){var _this=this;this.profile=$(profileIdentifier);this.inTransition=false;this.profileContent=this.profile.find('.right-panel');this.gradImage=this.profile.find('.grad-image');this.profileScrollPosition=null;this.workStrips=this.profile.find('.work-strip');this.worksContainer=$('.detailed-works');this.currentWork=null;this.hideProfile=function(){this.profile.addClass('show-work');this.profile.addClass('hide-profile-content');setTimeout(function(){_this.profileContent.hide();},400);}
this.unhideProfile=function(){$("html, body").animate({scrollTop:0});this.profileContent.show();this.profile.removeClass('show-work');setTimeout(function(){_this.profile.removeClass('hide-profile-content');},400);}
this.closeWorks=function(){this.currentWork=null;this.profile.addClass('hide-detailed-works');this.hideWorks();setTimeout(function(){_this.worksContainer.hide();},600);setTimeout(function(){_this.unhideProfile();},300);}
this.showWork=function(workID){var thisWork=this.worksContainer.find('.work-'+workID);var thisWorkCarousel=thisWork.find('.work-carousel');this.currentWork=workID;thisWork.show();setTimeout(function(){thisWork.addClass('view');},50);}
this.hideWorks=function(){var works=this.worksContainer.find('.work-container');works.removeClass('view');setTimeout(function(){works.hide();},400);}
this.openWorks=function(){_this.hideProfile();this.worksContainer.show();setTimeout(function(){_this.profile.removeClass('hide-detailed-works');},50);}
this.loadWork=function(workID){var thisWork=this.worksContainer.find('.work-'+workID);if(thisWork.length==0){var workURL=$('a[data-work-id="'+workID+'"]').attr('href');var workContent;this.worksContainer.append('<div class="work-container work-'+workID+'"></div>');thisWork=$('.detailed-works .work-'+workID);thisWork.load(workURL+' .work-content',function(){_this.showWork(workID);initialiseWorkPage(thisWork);});}else{_this.showWork(workID);}}
this.goToWork=function(workID,shouldPushState){$("html, body").animate({scrollTop:0});if(workID==null||workID==undefined){if(shouldPushState===true){history.pushState({work:null},'Profile');}
this.closeWorks();}else{if(shouldPushState===true){history.pushState({work:workID},'Work '+workID);}
this.openWorks();this.loadWork(workID);}}
window.onpopstate=function(event){if(event.state){_this.goToWork(event.state.work,false);}else{_this.goToWork(null,false);}}
this.updateGraduateProfileBlur=function(){this.profileScrollPosition=$(window).scrollTop();if(_this.profileScrollPosition<=50){$('#workJS').removeClass('work-show');$('#leftPanelJS').removeClass('work-show');}else{$('#workJS').addClass('work-show');$('#leftPanelJS').addClass('work-show');}}
$(window).scroll(function(){_this.updateGraduateProfileBlur();});this.updateGraduateProfileBlur();this.workStrips.click(function(e){e.preventDefault();_this.goToWork($(this).data('work-id'),true);});}
if($('#graduate-profile').length){var graduateProfile=new graduateProfile(('#graduate-profile'));}
if($('.single-work').length){initialiseWorkPage();}});})(jQuery);var $famID=$('#homepageFamJS'),imagePath=siteData.themeUri+'/assets/images/graduate-images-600x400/',prevGrads,gradUnique,newGrad,i=0,numberOfGrads=siteData.graduates.length,gradsJSON=siteData.graduates,gradRandom=function(min,max){return Math.floor((Math.random()*max)+min);};var currentGrads=[];for(i;i<6;i++){gradUnique=false;while(!gradUnique){newGrad=gradRandom(0,numberOfGrads-1);gradUnique=checkDupes(newGrad,currentGrads);}
currentGrads[i]=newGrad;var grad=gradsJSON[currentGrads[i]];console.log(grad);$famID.append('<li class="single-grad active"><a href="/graduate/'+grad.first_name+grad.last_name+'" class="grad-container"><img src="'+siteData.themeUri+'/assets/images/graduate-images-600x400/graduate-'+grad.ID+'.png"><span class="grad-name"><span class="board"></span><span class="name">'+grad.full_name+'</span></span></a></li>');prevGrads=currentGrads;}
function shuffleFam(){$('.grad-container img').fadeTo(300,0);$('.grad-container .grad-name').fadeTo(300,0).promise().done(function(){var currentGrads=[];var gradNew;i=0;for(i;i<6;i++){var loop=true;gradUnique=false;gradNew=false;while(loop){newGrad=gradRandom(0,numberOfGrads-1);gradUnique=checkDupes(newGrad,currentGrads);gradNew=checkDupes(newGrad,prevGrads);if(gradUnique&&gradNew){loop=false;}}
currentGrads[i]=newGrad;var grad=siteData.graduates[currentGrads[i]];$('#homepageFamJS').children(".single-grad:nth-of-type("+(i+1)+")").children().html("<img src='"+siteData.themeUri+"/assets/images/graduate-images-600x400/graduate-"+grad.ID+".png'><span class='grad-name'><span class='board'></span><span class='name'>"+grad.full_name+"</span></span>");}
prevGrads=currentGrads;});}
function checkDupes(n,currentGrads){var i=0;for(i;i<currentGrads.length;i++){if(currentGrads[i]==n){return false;}}
return true;}
$(document).ready(function(){$('body').addClass('loaded');var navInTransition=false;function openMobileMenu(){if(!navInTransition){navInTransition=!navInTransition;$('#primary-navigation-menu').show();setTimeout(function(){$('body').addClass('show-mobile-menu');navInTransition=!navInTransition;},50);}}
function closeMobileMenu(){if(!navInTransition){navInTransition=!navInTransition;$('body').removeClass('show-mobile-menu');setTimeout(function(){$('#primary-navigation-menu').hide();navInTransition=!navInTransition;},400);}}
$('.toggle-mobile-menu').click(function(e){if(!$('body').hasClass('show-mobile-menu')){openMobileMenu();}else{closeMobileMenu();}});$('#primary-navigation-menu').click(function(e){if($(window).innerWidth()<700&&e.target==this){console.log('yeh');closeMobileMenu();}});function Polygon(svg){this.arm={node:svg.find('polyline.a'),points:[],};this.reflection={node:svg.find('polyline.b'),points:[],};this.a.points=svg.find('polyline.a').attr('points').split(' ');this.b.points=svg.find('polyline.b').attr('points').split(' ');}
var scene=$('.scene').parallax();function updateDepths(){var w=$(document).innerWidth();var lastState;if(w>768){if(lastState!='desktop'){lastState='desktop';$('.layer').each(function(){$(this).attr('data-depth',$(this).data('desktop-depth'));scene.parallax('scalar',2,0);scene.parallax('updateLayers');});}}else{if(lastState!='mobile'){lastState='mobile';$('.layer').each(function(){$(this).attr('data-depth',$(this).data('mobile-depth'));scene.parallax('scalar',20,0);scene.parallax('updateLayers');});}}}
updateDepths();$(window).resize(updateDepths);function hideVideoOnMobile(){var iOS=/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream;if(iOS){$('video').hide();$('body').addClass('iOs');}}
hideVideoOnMobile();function watchSeen($collection){var scroll=$(document).scrollTop();var wh=$(window).innerHeight();$.each($collection,function(){var $this=$(this);var elTop=$this.offset().top;var elBottom=elTop+$this.outerHeight();if((elTop>=scroll&&elTop<=scroll+wh)||(elBottom>=scroll&&elBottom<=scroll+wh)){$this.addClass('seen');}});return $collection;}
var $scrollBasedAnimates=$('.panel-bridge, .horizon-rule, h2, .next-graduate');watchSeen($scrollBasedAnimates);$(window).scroll(function(){watchSeen($scrollBasedAnimates);});});