...
var $scrollBasedAnimates=$('.panel-bridge, .horizon-rule, h2');watchSeen($scrollBasedAnimates);$(window).scroll(function(){watchSeen($scrollBasedAnimates);});console.log(siteData.graduates.length);console.log(siteData.graduates);var $famID=$('#homepageFamJS'),imagePath=siteData.themeUri+'/assets/images/graduate-images-600x400/',prevGrads,gradUnique,newGrad,i=0,numberOfGrads=siteData.graduates.length,gradsJSON=siteData.graduates,gradRandom=function(min,max){return Math.floor((Math.random()*max)+min);};var currentGrads=[];for(i;i<6;i++){gradUnique=false;while(!gradUnique){newGrad=gradRandom(0,numberOfGrads-1);gradUnique=checkDupes(newGrad,currentGrads);}
currentGrads[i]=newGrad;var grad=gradsJSON[currentGrads[i]];$famID.append('<li class="single-grad active"><div class="grad-container"><img src="'+siteData.themeUri+'/assets/images/graduate-images-600x400/graduate-'+grad.ID+'.png"><span class="grad-name"><span class="board"></span><span class="name">'+grad.full_name+'</span></span></div></li>');prevGrads=currentGrads;}
function checkDupes(n,currentGrads){var i=0;for(i;i<currentGrads.length;i++){if(currentGrads[i]==n){return false;}}
return true;}});
