...
var $scrollBasedAnimates=$('.panel-bridge, .horizon-rule, h2');watchSeen($scrollBasedAnimates);$(window).scroll(function(){watchSeen($scrollBasedAnimates);});var $famID=$('#homepageFamJS'),imagePath=siteData.themeUri+'/assets/images/graduate-images-600x400/',prevGrads,gradUnique,newGrad,i=0,numberOfGrads=24;$.ajax({type:'GET',dataType:"json",url:siteData.themeUri+'/assets/data/temp-grads.json',success:function(objJSON){var currentGrads=[];for(i;i<6;i++){gradUnique=false;while(!gradUnique){newGrad=gradRandom(0,numberOfGrads-1);gradUnique=checkDupes(newGrad,currentGrads);}
currentGrads[i]=newGrad;var grad=objJSON[currentGrads[i]];$famID.append("<li class='single-grad'><a href='javascript:void(0)' class='grad-container'><img src='"+imagePath+grad.photoLink+".png'></img><span class='grad-name'>"+grad.name+"</span></a></li>");prevGrads=currentGrads;}}});$('#fam-reshuffle').click(function(){$('.grad-container img').fadeTo(300,0);$('.grad-container .grad-name').fadeTo(300,0).promise().done(function(){$.ajax({type:'GET',dataType:"json",url:siteData.themeUri+'/assets/data/temp-grads.json',success:function(objJSON){var currentGrads=[];var gradNew;i=0;for(i;i<6;i++){var loop=true;gradUnique=false;gradNew=false;while(loop){newGrad=gradRandom(0,numberOfGrads-1);gradUnique=checkDupes(newGrad,currentGrads);gradNew=checkDupes(newGrad,prevGrads);if(gradUnique&&gradNew){loop=false;}}
